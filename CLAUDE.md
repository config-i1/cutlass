...
3. ⚠ Persistent tile scheduler — **attempted, deadlocked**. Phase 6.3 attempt: convert the SM120 planar kernel-layer from blockIdx-driven one-tile-per-CTA to `PersistentTileSchedulerSm90` (static persistent, IsDynamicPersistent=false). Result: kernel hangs at runtime even for the trivial 1-tile case. Bisect isolated the issue to the persistent loop body itself (not the scheduler grid sizing nor the SharedStorage struct vs union change). The Phase 4b.2 mainloop's `load()` / `mma()` pipeline state machine appears to have implicit assumptions about being called once per CTA lifetime — cross-tile reuse of the same `PipelineTmaAsync` instance deadlocks. Resolving requires deeper rework of the mainloop's pipeline state management (likely modeling on SM90 cooperative's PipelineState handling with explicit prologue/mainloop/epilogue phase splits). Expected speedup vs Phase 6.1 was modest (5-15%) so not pursued further. Phase 6.1 remains the optimal shipping configuration.
